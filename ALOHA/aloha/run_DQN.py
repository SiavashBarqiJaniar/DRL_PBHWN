        ...
        if i%10000 == 0:
            for j in range(number_of_DRLs):
                ...
                for t, k in enumerate(output[j]): # s v a ratio p r
                    ...
                    #print(output[j][t][3])
                f.close()
                with open('rewards/agent' + str(j) + '_M20.txt', 'w') as f:
                    for k in reward_list:
                        f.write(str(k[j]) + '   ')
                with open('rewards/agent_action' + str(j) + '_M20.txt', 'w') as f:
                    for k in agent[j].action_list:
                        f.write(str(k) + '   ')
                with open('rewards/agent_buffer' + str(j) + '_M20.txt', 'w') as f:
                    for k in agent[j].buffer_history:
                        f.write(str(k) + '   ')
                with open('rewards/agent_collision' + str(j) + '_M20.txt', 'w') as f:
...
if __name__ == "__main__":
    ...
    for j in range(NN):
        agent.append(Agent(60, test, NN, 300))#60
