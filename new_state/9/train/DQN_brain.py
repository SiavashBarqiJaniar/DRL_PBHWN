...
class DQN:
	def __init__(self, 
		...
		self.gamma = gamma
		if test:
			self.epsilon = .05 #min(0.01, epsilon)
	...
	def build_model(self):
		...
		h1 = Dense(64, activation="relu", kernel_initializer=glorot_normal(seed=247))(inputs) #h1
		h2 = Dense(64, activation="relu", kernel_initializer=glorot_normal(seed=2407))(h1) #h2
		outputs =  Dense(self.n_actions, kernel_initializer=glorot_normal(seed=242147))(h2)
		model = Model(inputs=inputs, outputs=outputs)
		model.compile(loss="mse", optimizer=RMSprop(lr=self.learning_rate))
		return model
	...
	def choose_action(self, state):
		...
		if np.random.random() < self.epsilon:
			return np.random.randint(0, 2), action_values[0]
