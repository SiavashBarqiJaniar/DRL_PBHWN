...
sum_line, = plt.plot(sum_throughputs, color='y', lw=1, label='sum')
...
for j in range(numberOfDRLs):
    ...
    for x in range(2 - j):
        col += '00'
    axs[0].plot(agent_throughputs[j], color=col, lw=1)
    axs[0].plot(agent_actions[j]*(.5 + j/2 + agent_collisions[j]/5) + 1 + j/10, '.', color=col, lw=.1, label='agent'+str(j))
