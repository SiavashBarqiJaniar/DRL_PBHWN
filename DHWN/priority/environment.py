...
class ENVIRONMENT(object):
    ...
    def step(self, action, agent):
        ...
        if cap == 0:
            for j in range(n):
                if action[j] == 1:
                    ...
                    if agent[j].threes > 0:
                        opponent_priority = 3
                        opponent_packets = agent[j].threes
                    elif agent[j].twos > 0:
                        opponent_priority = 2
                        opponent_packets = agent[j].twos
                    else:
                        opponent_priority = 1
                        opponent_packets = agent[j].ones
        ...
        for j in range(n):
            ...
            elif action[j] == 0 and cap == 0:
                if agent[j].threes > 0:
                    priority = 3
                    packets = agent[j].threes
                elif agent[j].twos > 0:
                    priority = 2
                    packets = agent[j].twos
                else:
                    priority = 1
                    packets = agent[j].ones
                if priority > opponent_priority:
                    agent_reward[j] = 0
                    reward[j] = 0
                    ack[j] = 0
                    opp_id = opponent_id
                    if opponent_id > j:
                        opp_id -= 1
                    agent[j].opps_buffers[opponent_priority-1][opp_id] = opponent_packets
                elif priority == opponent_priority:
                    if packets > 0 and packets > opponent_packets:
                        agent_reward[j] = 0
                        reward[j] = 0
                        ack[j] = 0
                        opp_id = opponent_id
                        if opponent_id > j:
                            opp_id -= 1
                        agent[j].opps_buffers[priority-1][opp_id] = opponent_packets
                    elif packets > 0 and packets <= opponent_packets:
                        agent_reward[j] = 0
                        reward[j] = 1#2
                        ack[j] = 0#2
                        opp_id = opponent_id
...
# =============================================================================
        ...
        for j in range(n):
            ratio = [agent[j].one_ratio, agent[j].two_ratio, agent[j].three_ratio]
            maxim = max(ratio[opponent_priority-1])
